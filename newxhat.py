...
class L298NMDc(object):
    ...
    def __init__(
        self, pin_one, pin_two, pwm_pin, freq=50, verbose=False, name="DCMotorX"
    ):
        """ init method
...
MotorThree = L298NMDc(port.PA7, port.PA8, port.PA19, 50, False, "motor_three")
MotorFour = L298NMDc(port.PA9, port.PA10, port.PA20, 50, False, "motor_four")
...
def motor_three_speed(speed):
    try:
        if speed < 0:
            MotorThree.backward(-speed)
        elif speed > 0:
            MotorThree.forward(speed)
        else:
            MotorThree.stop(0)
            # print("motor stopped\n")
...
def motor_four_speed(speed):
    try:
        if speed < 0:
            MotorFour.backward(-speed)
        elif speed > 0:
            MotorFour.forward(speed)
        else:
            MotorFour.stop(0)
            # print("motor stopped\n")
...
if __name__ == "__main__":
    while True:
        motor_one_speed(30)
        motor_two_speed(30)
        # activates more motors!!
        motor_three_speed(30)
        motor_four_speed(30)
